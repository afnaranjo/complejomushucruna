...
### 2026-09-24 — Landing de Mushuc Freestyle 2026

- Alex entregó el arte `post (1) a.rar` (un SVG de 7,5 MB) como línea gráfica y pidió la landing. Se creó `/finados/mfs/` (noindex) y la opción `MFS` al final del submenú de Finados, según la especificación aprobada del módulo.
- Del SVG se extrajeron, sin redibujar: logo «mushuc freestyle», ícono MFS, letras de fondo, organizadores y Plaza de la Luna como SVG vectoriales limpios (sin rasters ni scripts); el retrato en semitono como WebP transparente; textura de fondo y el arte completo renderizado con su tipografía. Badeen Display (Google Fonts, licencia OFL) se sirve localmente. El retrato proviene de un archivo marcado en el propio SVG como generado con IA; se describe como ilustración.
- Contenido tomado solo de la especificación y del arte: final lunes 2 de noviembre 13:00 en la Plaza de la Luna, cierre jueves 29 de octubre 23:59, 32 cupos, premios USD 600/250/100 con sus condiciones, medallas, reconocimientos y trofeos, reconocimiento al mejor MC de la zona centro y contacto de auspicios. El registro en línea todavía no existe: la página lo anuncia como «muy pronto» y no enlaza a un formulario inexistente.
- QA: `npm run check` completo en verde (214 Node, PHP, 10 integración; 203 archivos, 66 HTML, 2012 referencias). Revisión visual en 1440 px y 375 px sin desbordamiento horizontal.
- Publicación externa: quedó publicada el 2026-09-24 como parte del despliegue frontend autorizado para actualizar la franja de auspiciantes; `/finados/mfs/` respondió HTTP 200 en ambos dominios. Riesgos: confirmar con el frente dueño fechas, premios, cupos y el acceso con entrada general antes de difundir; construir cuentas y registro (migración 028) según el plan.

### 2026-09-24 — Nuevos auspiciantes y animación tipo telón

- Se integró `origin/main` por avance rápido antes de editar. Alex entregó el nuevo `logos auspiciantes.svg` oficial de `2321 × 650`, que incorpora SanFra y Bogati; el maestro SVG y el WebP optimizado sustituyen la versión anterior en la franja compartida de FINADOS y SHOWS.
- La alternativa elegida para evitar la animación anterior es una apertura desde el centro tipo telón, ejecutada una sola vez al entrar en pantalla. No hay movimiento continuo y `prefers-reduced-motion` muestra la franja sin transición. Caché `20260924-sponsors-7`.
- La revisión visual en escritorio y móvil detectó que el recorte del nodo observado impedía disparar el `IntersectionObserver`; se corrigió observando el contenedor visible y animando únicamente el arte hijo. FINADOS y SHOWS quedaron sin desbordamiento horizontal.
...
### 2026-09-24 — Inscripción de Mushuc Freestyle con cuentas y panel administrativo

- Alex pidió activar el registro de participantes, con su sección en el panel debajo de Creadoras, sin dañar lo que ya funciona. Se construyó como módulo aislado sobre el patrón probado de Emprendedores: tablas `mfs_*` (migración aditiva `028_mfs_accounts`, solo crea tablas), sesión `finados_mfs`, almacén cifrado `mfs-photos/`, catálogo `mfs-consents.json` y clases `MfsAuth`, `MfsPasswordReset`, `MfsPhotoStorage` y `MfsRepository`, cargadas solo cuando se usa una ruta de MFS.
- Participante: `/finados/mfs/acceso/`, `/mi-registro/` y `/restablecer/` (noindex, sin Pixel ni aviso de cookies). Ficha con nombres, nombre artístico opcional, WhatsApp cifrado, foto tipo retrato obligatoria, enlace HTTPS de TikTok (fijo una vez enviado), declaración del video y tres consentimientos. Documentos `/finados/mfs/bases/` y `/finados/mfs/politica-de-privacidad/` con el mismo responsable de los demás programas. La landing ahora enlaza a la cuenta.
- Administración: `/admin/mfs/` («Mushuc Freestyle», debajo de Creadoras): resumen, búsqueda por nombre, correo o WhatsApp, estados (Nuevo, En revisión, Aprobado, Rechazado, Seleccionado), foto privada, corrección de audición, notas, retiro reversible con restauración, cuentas pendientes, enlace de recuperación y CSV.
- Cambios en archivos compartidos, solo aditivos: ámbito `mfs` en `Http.php`, rutas `/api/mfs/*`, `/api/mfs-participants*`, `/api/mfs-accounts*` y `/api/mfs-dashboard` en `Router.php`, lista del artefacto de despliegue, build y el ítem del sidebar. Voceros, Medios, Emprendedores y Creadoras no cambian.
- QA: `npm run check` en verde (218 Node, 26 suites PHP con `mfs_accounts_test.php`, 10 integración; 211 archivos, 72 HTML, 2138 referencias). Flujo real por HTTP en el entorno local: cuenta, login, ficha sin foto rechazada, ficha con foto guardada, audición bloqueada, foto propia, lista, resumen, foto admin, estado, retiro, restauración, y Voceros y Emprendedores respondiendo igual. Se actualizaron expectativas de listas fijas (orden del sidebar, conteo de páginas, exclusiones de Pixel y cookies, paridad dev/producción) sin debilitarlas.
- Pendientes: validar los textos legales y el tratamiento de menores de edad con el responsable; confirmar fechas, premios y cupos con el frente dueño.
- Publicación externa autorizada: commit `497f20e` en `origin/main` (integrado por rebase sobre dos commits de otra sesión, conservando ambas entradas de bitácora). Backend con respaldo, migración 028 aplicada y activación correcta; frontend con respaldo y sin borrar archivos exclusivos. El primer intento se detuvo en las pruebas previas, sin tocar el servidor, por 12 copias duplicadas « 2» creadas por iCloud, idénticas a sus originales; se movieron a `~/Downloads/respaldo-duplicados-icloud-mfs-20260924/` sin borrar nada. Verificación por el espejo: las seis páginas de MFS y `/admin/mfs/` responden 200, los scripts publicados son idénticos al build (incluidos `vocero-portal.js`, `admin.js` y `admin-creadoras.js` sin cambios), `health` 200, sesión MFS 200 y rutas administrativas sin sesión 401. Sin cambios en datos existentes, DNS, Google Sheets ni otros proyectos.

### 2026-09-24 — Botón «Iniciar sesión» en la portada de Mushuc Freestyle
...
### 2026-09-24 — Enlaces en pestaña nueva cuando la portada se muestra incrustada
...
### 2026-09-24 — Redes sociales en el Panel (Metricool, solo Finados Mushuc Runa)

- Alex pidió en el Panel una sección de redes sociales con selector de fechas (por defecto 30 días) e indicadores para decidir si se mejora: alcance, seguidores, pauta, crecimiento por red e interacción; precisó que solo debe mostrar la cuenta Finados Mushuc Runa.
- Backend: `SocialMetrics.php` (carga bajo demanda) y `GET /api/redes-sociales?from&to[&refresh=1]` con sesión administrativa. Consulta Metricool en solo lectura, en paralelo, con el token en cabecera; la marca 5238051 está fija en código y otra en la configuración se rechaza. Compara con el periodo anterior del mismo largo y guarda caché privada de 30 minutos. Sin migraciones ni cambios de datos.
- Frontend: sección al inicio de `/admin/panel/` con cifras totales, lectura rápida, una tarjeta por red (seguidores, crecimiento, mini-gráfico diario, métricas con comparación y veredicto) y Meta Ads. Cachés `panel.js` `20260924-admin-panel-5`, `admin-medios.js` `20260924-admin-medios-21` (solo suma la ruta permitida) y `admin.css` `20260924-17`.
...
- Publicado con autorización de Alex («sube»): frontend con respaldo y sin borrar archivos exclusivos; en ambos dominios el Panel entrega las cuatro secciones plegadas y `panel.js`, `admin.css` y `vocero-portal.js` coinciden por SHA-256 con el build.

### 2026-09-25 — Calendario estratégico de comunicación dentro de Medios

- Alex entregó `calendario_estrategico_comunicacion_2026.html` (herramienta suelta con spots, calendario semanal de arrastrar y soltar, medios de pauta, plan de medios y reportes PNG/PDF, guardada solo en el navegador) y pidió integrarla en el panel de Medios con la línea gráfica de Finados, en el sidebar al hacer clic en Medios y sin borrar nada.
- Nueva página `/admin/medios/calendario/` («Calendario de medios», debajo de Eventos dentro de Medios). Conserva las cinco pestañas y todas sus funciones: alta/edición/eliminación de spots, estrategia sugerida, restaurar y limpiar, arrastrar audios y mover pautas entre semanas, medios de pauta, plan con impactos e inversión, reportes, respaldo JSON e importación.
- Cambios de adaptación: los datos ahora se guardan en el servidor para todo el equipo (antes solo en un navegador), con autoguardado y control de versión para no pisar cambios ajenos; colores de Finados; botón «+ Agregar a esta semana» para celular; un medio de pauta puede traerse desde Medios registrados. Como la política de seguridad del panel no permite librerías externas, el PNG y el PDF se generan con código propio (canvas y un PDF de una página) en lugar de html2canvas/jsPDF. El respaldo JSON del archivo original se puede importar.
- Backend: migración aditiva `030_media_plan` (una sola fila con el documento, versión, autor y fecha), `MediaPlanStore.php` con validación estricta y ruta `GET/POST /api/media-plan` con sesión administrativa y CSRF; auditoría `media.plan_saved`. Medios, Eventos y Voceros no cambian.
...
- Publicado con autorización de Alex («sube a produccion»): frontend con respaldo y sin borrar archivos exclusivos. En ambos dominios `/admin/medios/` entrega el botón de Medios y «Seguimiento de medios» como página actual; `sidebar.js`, `admin.css`, `admin-medios.js`, `admin.js` y `vocero-portal.js` coinciden por SHA-256 con el build; Panel, Eventos, Calendario y Voceros responden 200.

### 2026-09-25 — Fluidez del sitio y del panel, y calendario de medios sin Medios ni Plan

- Alex reportó que las páginas no se sentían fluidas: al entrar, el contenido «se baja» mientras carga. Medición en producción: cada petición tarda ~0,6 s en responder; CSS, JavaScript y fuentes no tenían instrucción de caché ni compresión, los módulos se cargaban en cadena (4–5 esperas seguidas) y la banda del tema central y el menú de Medios cambiaban de tamaño después de cargar.
- `.htaccess` (bloque propio, protegido con `IfModule` y conservando las directivas de cPanel): compresión gzip de texto; archivos con `?v=` en caché por 1 día con renovación en segundo plano (sin `immutable`, para que un olvido de versión se corrija solo); fuentes 30 días; imágenes sin versión 1 día; HTML y archivos sin versión `no-cache`. Se probó en una carpeta temporal del servidor antes de publicar, sin tocar el sitio, y se retiró después: Apache lo acepta y la compresión reduce el CSS de 64 a 13,5 KB, el JS de 75 a 19 KB y el HTML de 19 a 5 KB.
- Build: cada página anuncia en el `<head>` todo su árbol de módulos (`modulepreload`); las administrativas además adelantan la fuente y la conexión con la API. Los módulos que se importaban sin versión (`runtime-origins.mjs`, `qrcode-generator.mjs`) reciben automáticamente una versión según su contenido. Precarga al pasar el cursor por enlaces internos (Speculation Rules, sin la API) y transición suave entre páginas con encabezado y menú fijos.
- Sin saltos: el grupo de Medios llega plegado o abierto desde el servidor; la banda del tema central reserva su espacio y se pinta al instante con la última versión vista en esa pestaña (solo tramos y avisos de campaña, sin datos personales). Seguimiento de medios y Panel piden sesión y datos a la vez.
- Calendario de medios: por pedido de Alex se retiran las pestañas «Medios de comunicación» y «Plan de medios» (ya cubiertas por Seguimiento de medios) y su reporte; quedan Spots, Calendario semanal y Reportes. Lo ya guardado de esas pestañas no se borra: se conserva en el documento y en el respaldo.
- QA: `npm run check` en verde (236 Node, PHP, 10 integración; 218 archivos, 74 HTML, 2540 referencias) con prueba nueva de caché, compresión, precarga y módulos versionados. Sin cambios de backend ni de datos.
- Corrección del menú lateral reportada por Alex con captura: en escritorio, dentro de `<details>` la altura del 100 % no se aplicaba y, cuando el menú era más alto que la ventana (más aún con las tres opciones de Medios), el contenido se salía del fondo morado. Ahora tiene altura máxima propia, desplazamiento interno y fondo propio; medido a 673 y 913 px de alto, el contenido queda dentro y se puede desplazar. Caché `admin.css` `20260925-22`.
- Publicado con autorización de Alex («retíralo»): frontend con respaldo y sin borrar archivos exclusivos. En ambos dominios el calendario muestra solo Spots, Calendario semanal y Reportes (sin menciones a Medios ni Plan); CSS y JS llegan comprimidos con caché de un día; HTML con `no-cache` y reglas de precarga; los archivos publicados y el `.htaccess` del servidor coinciden por SHA-256 con el build (el `.htaccess` sigue bloqueado al público, 403); ocho rutas públicas y administrativas responden 200 y la API sigue en 200.

### 2026-09-25 — Guion de la voz y audio por spot; diálogo centrado; corrección del guardado del calendario

- Alex reportó que el diálogo «Editar spot» se abría arriba a la izquierda y pidió, en cada spot, un espacio para el guion de la voz y la opción de subir el audio y descargarlo.
- Diálogo: el reinicio de estilos le quitaba el margen automático; ahora se centra, tiene alto máximo con desplazamiento y encabezado y botones fijos.
- Spot: campo «Guion de la voz» (hasta 8.000 caracteres, conserva saltos de línea) y bloque de audio para subir, reemplazar, escuchar en el mismo panel y descargar. En un spot existente el audio queda guardado en el acto; en uno nuevo, al guardar el spot. La tabla muestra «Guion ✓» y un botón de descarga.
- Backend: `POST /api/media-plan/audio` (multipart, hasta 30 MB, tipo verificado por el contenido: MP3, WAV, M4A, AAC, OGG, Opus, WebM o FLAC) y `GET /api/media-plan/audio/{id}` (adjunto, sin caché). Los audios se guardan en la carpeta privada `media-plan-audio/`, fuera de la parte pública, y solo se descargan con sesión administrativa; se audita cada subida. Sin migraciones: el guion y la referencia al audio viven en el documento del calendario. La política de seguridad del panel suma `media-src blob:` para escuchar.
- Corrección importante: las rutas del calendario usaban constantes de `MediaPlanStore` antes de cargar la clase; en producción, donde cada petición arranca de cero, el primer guardado del calendario habría respondido 500. Las pruebas unitarias no lo detectaban porque la clase ya estaba cargada. Además, el punto de entrada leía como máximo 16 KB del cuerpo y un calendario con guiones lo supera; ahora esa ruta tiene su propio tope de 512 KB. Nadie había guardado todavía el calendario, así que no se perdió nada.
- QA: `npm run check` en verde (236 Node, suites PHP con audio y guion, 11 de integración). Prueba de integración nueva por HTTP real: calendario de más de 16 KB, subida multipart, descarga idéntica, rechazo de archivos que no son audio y 401 sin sesión. Revisión visual del diálogo y la tabla.
- Riesgo: la carpeta `media-plan-audio/` no está en el respaldo verificado de la aplicación (igual que `media-photos/`).
- Publicado con autorización de Alex («súbelo a producción»): backend (verificación completa, respaldo, sin migraciones) y luego frontend (respaldo, sin borrar archivos exclusivos). Verificación: en ambos dominios el diálogo del spot trae guion, subida y descarga de audio, la columna «Guion y audio» y `media-src blob:`; cinco archivos con SHA-256 idéntico al build; `health` 200 y `/api/media-plan` y `/api/media-plan/audio/{id}` sin sesión 401 en ambas APIs. En el servidor, el Router publicado carga la clase antes de usarla, el punto de entrada activo (`current/public/index.php`, al que redirige el docroot) tiene el tope de 512 KB y `finfo` está disponible.

### 2026-09-25 — Varios guiones por spot

- Alex pidió que un spot pueda tener más de un guion, verlos con un clic y agregar otros. Cada spot tiene ahora una lista de guiones (hasta 20), cada uno con título, texto (hasta 8.000 caracteres) y su propio audio para subir, escuchar, descargar, reemplazar o quitar. Cada guion es una tarjeta plegable; «+ Agregar guion» suma otro. La tabla muestra cuántos guiones y audios tiene cada spot y el botón «Ver guiones».
- Un spot guardado con el formato anterior (un solo guion y un audio) pasa a ser su primer guion, tanto en el servidor como al cargar y al importar un respaldo; no se pierde nada. Subir o quitar un audio en un spot existente se guarda en el acto; lo demás, con «Guardar spot».
- Backend: `scripts` validado en `MediaPlanStore` (ids únicos, título, texto, referencia de audio); el tope del documento sube a 1 MB (en la clase y en el punto de entrada). Sin migraciones.
- QA: `npm run check` en verde, con pruebas PHP de conversión, varios guiones, límites e ids repetidos, y la integración HTTP con dos guiones largos por spot y audio en el segundo. Revisión visual del diálogo y la tabla.
- Publicado con autorización de Alex («súbelo»): backend (verificación completa, respaldo, sin migraciones) y frontend (respaldo, sin borrar archivos exclusivos). En ambos dominios el diálogo trae la lista de guiones y «+ Agregar guion»; cuatro archivos con SHA-256 idéntico al build; `health` 200 y `/api/media-plan` sin sesión 401; en el servidor, el backend activo admite 20 guiones por spot y el punto de entrada lee hasta 1 MB para el calendario.

### 2026-09-25 — Gira de medios dentro de Medios

- Alex pidió un submenú «Gira de medios» dentro de Medios, con un calendario como el de Creadoras: agregar personas, elegir un medio de Seguimiento y asignárselo a esa persona.
- Página `/admin/medios/gira/` (en el menú, dentro de Medios, después de Calendario de medios). Personas de la gira (nombre, rol, teléfono cifrado y nota); calendario con vistas día/semana/mes que reutiliza las funciones y estilos de Creadoras; cada cita reúne el medio de Seguimiento (con buscador por nombre, ciudad o frecuencia), quién va (una o varias personas), día, horas, tipo (entrevista, en vivo, grabación, visita, rueda de prensa, otro), estado (programada, confirmada, realizada, no se dio), lugar y nota. Se agenda tocando una hora, tocando un nombre o arrastrándolo al calendario; las cajas se arrastran y se estiran. Resumen arriba: citas, confirmadas, realizadas, medios visitados y personas.
- Backend: migración aditiva `031_media_tour` (tablas `media_tour_people`, `media_tour_visits` y `media_tour_visit_people`), `MediaTourRepository.php` y rutas `/api/media-tour*` con sesión administrativa y CSRF. Una persona no puede tener dos citas a la misma hora (409). Quitar una cita o retirar a una persona no borra: queda marcada y se conserva el historial. Todo queda en la auditoría.
- QA: `npm run check` en verde (238 Node, suites PHP con `media_tour_test.php`, 11 integración; 220 archivos, 75 HTML, 2576 referencias). Revisión visual del calendario y del diálogo.
- Publicado con autorización de Alex («súbelo»): backend (verificación completa, respaldo y migración 031 aplicada) y frontend (respaldo, sin borrar archivos exclusivos). Verificación: `/admin/medios/gira/` 200 en ambos dominios con el enlace dentro de Medios; cinco archivos con SHA-256 idéntico al build; Seguimiento, Eventos, Calendario, Creadoras y Voceros siguen en 200; `health` 200 y `/api/media-tour` sin sesión 401. En la base real: migración registrada, tres tablas nuevas vacías y la consulta del calendario funciona con los 72 medios de Seguimiento disponibles para elegir (probada en una transacción de solo lectura). Voceros intacto (133 fichas).
- Ajuste pedido por Alex con captura: «Buscar medio» y «Medio de Seguimiento» eran dos campos para lo mismo y confundían. Ahora hay un solo campo «Medio»: se escribe el nombre, la ciudad o la frecuencia, las coincidencias de Seguimiento aparecen debajo (con ratón o con flechas y Enter) y el elegido queda marcado con ✓. Si se cambia lo escrito, hay que volver a elegir de la lista. Solo frontend; `npm run check` en verde.
- Publicado con autorización de Alex («sí»): frontend con respaldo y sin borrar archivos exclusivos. En ambos dominios la Gira muestra el campo único con lista de coincidencias y ya no el buscador y el selector separados; archivos con SHA-256 idéntico al build.

### 2026-09-25 — Producción: Activaciones, Cronograma Sol y Cronograma Luna

- Alex pidió un grupo «Producción» en el menú con Activaciones, Cronograma Sol y Cronograma Luna, cada uno independiente, con calendario como Gira de medios y cajas que se arrastran como en Calendario de medios. Confirmó por pregunta: días con horas.
- Grupo «Producción» (botón que despliega, como Medios) con `/admin/produccion/activaciones/`, `/admin/produccion/sol/` y `/admin/produccion/luna/`. Una sola plantilla para los tres: biblioteca de piezas a la izquierda (nombre, descripción, color y duración por defecto) que se arrastran a una hora del calendario y quedan agendadas al soltarlas; calendario día/semana/mes con el motor de Creadoras y Gira; cajas con el color de su pieza que se mueven y se estiran; diálogo del bloque con título, día, horas, color, estado (planificado, confirmado, realizado, no se hizo), responsable, lugar y nota. Resumen: piezas, bloques, confirmados y realizados.
- Backend: migración aditiva `032_production_boards` (tablas `production_items` y `production_entries`, separadas por tablero), `ProductionRepository.php` y rutas `/api/produccion/{activaciones|sol|luna}/…` con sesión administrativa y CSRF. Los tableros no se mezclan (una pieza de un tablero no se usa en otro). Archivar una pieza la saca de la biblioteca y conserva sus bloques; quitar un bloque no lo borra. Todo queda en la auditoría.
- Por pedido de Alex («terminan después de las 12»), en Producción cada día del calendario es una jornada de 06:00 a 03:00 del día siguiente: un show de 23:00 a 01:30 es una sola caja en la noche en que empieza, y lo que empieza de madrugada pertenece a la noche anterior. El diálogo interpreta una hora de fin de madrugada como del día siguiente; un bloque dura como máximo 12 horas. El backend admite bloques que terminan al día siguiente y consulta cada rango de 06:00 a 06:00. Gira y Creadoras no cambian.
- QA: `npm run check` en verde (240 Node, suites PHP con `production_boards_test.php`, 11 integración; 224 archivos, 78 HTML, 2696 referencias). Revisión visual del Cronograma Sol con biblioteca y bloques.
- Publicado con autorización de Alex («súbelo»): backend (verificación completa, respaldo y migración 032 aplicada) y luego frontend (respaldo, sin borrar archivos exclusivos). Verificación: los tres tableros responden 200 en ambos dominios con el grupo Producción en el menú y la jornada de 06:00 a 03:00; cinco archivos con SHA-256 idéntico al build; Medios, Gira, Creadoras y Voceros siguen en 200; `health` 200 y `/api/produccion/sol` sin sesión 401. En la base real: migración registrada y la consulta de los tres tableros funciona, vacíos (probado en una transacción de solo lectura). Voceros intacto (133 fichas).

### 2026-09-26 — Creadoras: menú desplegable, sección Edición y calendario por semanas

- Alex pidió que `Creadoras` en el menú administrativo despliegue dos opciones, `Creadoras` y `Edición`. En Edición deben aparecer los videos que se grabaron: si el video tuvo guion, el editor lo ve completo; si no lo tuvo, se muestra como creado nuevo. El editor lo marca como editado. También pidió que el calendario de Creadoras se parezca al Calendario de medios (vista por semanas), sin borrar los guiones ni los turnos ya agendados.
- Menú: `Creadoras` pasa a ser un botón que despliega, como Medios y Producción, con `Creadoras` (`/admin/creadoras/`) y `Edición` (`/admin/creadoras/edicion/`).
- Calendario: se abre en la nueva vista **Semanas**. Muestra siete columnas desde la semana del 21 de septiembre, con los nombres a la izquierda. Un turno se arrastra a otra semana y conserva su día y su hora; «+ Agregar a esta semana» abre el turno; soltar un nombre sobre una semana abre el turno con esa creadora, y sobre un turno la suma a esa caja. También se puede copiar y pegar un turno en otra semana. Las vistas por horas (Día, Horario semanal y Mes), el diálogo del turno, los guiones, la asistencia y la bitácora no cambian. Los totales quedan arriba y los indicadores por creadora pasan a una sección plegable.
- Edición: la cola reúne cada contenido registrado en un turno y cada guion marcado como grabado que todavía no tiene video ligado. Tiene resumen clicable (por editar, ya editados, con guion, creados nuevos, total), filtros por estado, creadora, guion y texto, el guion completo con sus saltos de línea y su referencia, la corrección de «¿salió de un guion?», el botón «Marcar como editado» con fecha y autor, y los campos de enlace del video editado y nota. Al registrar contenido en un turno ahora se elige de qué guion salió («Sin guion · creado nuevo» por defecto).
- Backend: migración aditiva `033_creadora_editing`, que solo agrega columnas (`script_id`, `edited_at`, `edited_by`, `edited_url` y `edit_note` en contenido; los cuatro campos de edición en guiones) y no modifica ni borra datos. Rutas nuevas con sesión administrativa y CSRF: `GET /api/creadoras/edicion` y `PATCH /api/creadoras/edicion/(contenido|guion)/{id}`. Quitar un guion ya no deja a un video apuntando a él: el video se conserva sin guion. Cada marca queda registrada en la bitácora del calendario y en la auditoría.
- QA: `npm run check` completo en verde, con 246 pruebas Node (5 nuevas en `creadoras-edicion.test.mjs`), 32 suites PHP (`creadora_editing_test.php` nueva) y 11 de integración; build de 226 archivos, 79 HTML y 2738 referencias. Se probó el flujo real por HTTP en el entorno local con datos de ejemplo: cola con 4 videos (2 con guion y 2 creados nuevos) y marca de editado. Se actualizaron las expectativas del orden del menú y de la vista inicial. No se hizo revisión visual en el navegador desde esta sesión.
- Publicación externa: Alex autorizó con «si». Commit `43dd5c8` en GitHub. El backend se publicó primero, con verificación completa, respaldo y la migración 033 aplicada; después el frontend, con respaldo y sin borrar archivos exclusivos del servidor. Verificación en el espejo: `/admin/creadoras/`, `/admin/creadoras/edicion/`, Voceros y Medios responden 200; `admin-creadoras.js`, `admin-creadoras-edicion.js`, `admin.css` y `vocero-portal.js` coinciden por SHA-256 con el build; el calendario abre en Semanas; `health` responde 200 y `/api/creadoras/edicion` sin sesión responde 401. El dominio principal no resolvió desde esta red, pero el despliegue reportó verificación correcta. No se modificaron datos existentes, DNS, Google Sheets, Meta ni otros proyectos.
- Pendiente: revisión visual de Alex. Los videos que ya están registrados sin guion aparecen como creados nuevos; si salieron de un guion, se ligan desde Edición con «¿Salió de un guion del turno?».

### 2026-09-26 — Creadoras: solo horario semanal y arrastre como en el Calendario de medios

- Tras ver la vista Semanas publicada, Alex aclaró que solo necesita el **Horario semanal** (por horas), pero con la forma de arrastrar las cajas del Calendario de medios.
- Se retiró la vista Semanas, con sus funciones y sus pruebas. El calendario vuelve a abrir en Horario semanal; Día y Mes siguen disponibles.
- Arrastre nuevo: la caja viaja con el puntero, ligeramente inclinada y con sombra. Una sombra punteada del color de la creadora marca en la columna exacta dónde va a caer, y la columna destino se resalta. La hora se toma del borde superior de la caja, no del puntero. Al arrastrar un nombre de la lista, la sombra muestra el turno de 3 horas que se creará. Se quitó el `title` de las cajas, cuyo cuadro del navegador tapaba el arrastre, y se reemplazó por un `aria-label`. El calendario es más alto (60rem; 48rem en móvil) para agarrar mejor las cajas. Soltar en el mismo lugar ya no guarda nada.
- Edición y la migración 033 no cambian.
- QA: `npm run check` completo en verde (246 Node, suites PHP y 11 de integración; 226 archivos, 79 HTML y 2738 referencias). No se hizo revisión visual en el navegador desde esta sesión.
- Publicación externa: Alex autorizó con «si subelo». Commit `80a8ef5` en GitHub y frontend publicado con respaldo previo, sin borrar archivos exclusivos del servidor; el backend no cambió. En el espejo: Creadoras, Edición y Voceros responden 200; `admin-creadoras.js`, `admin.css`, `admin-creadoras-edicion.js` y `vocero-portal.js` coinciden por SHA-256 con el build; el calendario abre en Horario semanal y ya no ofrece la vista Semanas.

### 2026-09-26 — Usuarios y roles de acceso al panel administrativo

- Alex pidió que cada persona entre al panel con su propia cuenta y un rol que limite lo que ve: Administración ve todo; Comunicación, solo Medios; Community, solo Creadoras; Producción, solo Producción; el Panel, solo Administración. Él activa en cada rol los módulos que se vayan creando. Motivo: hoy todos entran con `admin` y no se puede saber quién hizo cada cambio.
- Backend: migración aditiva `034_admin_roles`. Crea las tablas `admin_roles` y `admin_setup_tokens` y agrega a `admin_users` las columnas `role_id`, `full_name` y `created_by_admin_id`. Siembra los roles Administración (del sistema, ve todo, también módulos futuros), Comunicación, Community y Producción, y asigna la cuenta `admin` a Administración. Clase nueva `AdminUsers.php`, con el catálogo de módulos y el mapa ruta→módulo.
- Inicio de sesión: `Auth` acepta cualquier cuenta activa (antes solo `admin`) y carga el rol en cada petición, así un cambio de rol o una desactivación rige de inmediato. La cuenta `admin` sin rol conserva acceso total y no se puede desactivar ni cambiar de rol. Antes de publicar se confirmó, con un conteo de solo lectura, que en producción existe una única cuenta (`admin`).
- Control de acceso: el servidor responde 403 en cada ruta de un módulo que el rol no tiene; no depende solo del menú. La banda del tema central (`GET /api/noticias`) la leen todos. El menú lateral oculta los módulos ajenos y, si alguien abre una sección que no es de su rol, lo lleva a su portada. Tras iniciar sesión, cada persona entra a la portada de su rol.
- Contraseñas: la persona que administra no escribe ni conoce la contraseña de nadie. Al crear una cuenta se genera un enlace de un solo uso, válido 72 horas (`/admin/activar/?token=…`), con el que la persona elige su contraseña (mínimo 10 caracteres). El token se guarda solo como SHA-256 y generar un enlace nuevo anula el anterior.
- Página `/admin/usuarios/` («Usuarios», al final del menú): lista de personas con cambio de rol inmediato, activar/desactivar y enlaces de contraseña; roles con casillas de módulos (el de Administración queda bloqueado y ve todo); y **Actividad**, que muestra quién hizo qué y cuándo, en hora de Ecuador, a partir de la auditoría. Siempre debe quedar al menos una cuenta que administre usuarios.
- QA: `npm run check` completo en verde, con 251 pruebas Node (5 nuevas en `usuarios.test.mjs`), 33 suites PHP (`admin_roles_test.php` nueva) y 11 de integración; build de 230 archivos, 81 HTML y 2794 referencias. Se actualizaron las expectativas del orden del menú, del texto del rol (`Administración`), de la redirección tras el inicio de sesión y del `sidebar.js` publicado. No se hizo revisión visual en el navegador.
- Publicación externa: Alex autorizó con «sube». Commit `1c352ab`. Se publicó primero el backend, con verificación completa, respaldo y la migración 034 aplicada, y después el frontend, con respaldo y sin borrar archivos exclusivos del servidor. En el espejo verificado: responden 200 los portales públicos (Voceros, Medios, Emprendedores y MFS), `/admin/`, `/admin/usuarios/` y `/admin/activar/`; también responden 200 `health` y las sesiones de los cinco portales. El registro de Voceros recibe y valida (422 con datos vacíos, sin crear nada). `/api/voceros` y `/api/admin-users` sin sesión responden 401. `vocero-portal.js`, `admin.js`, `sidebar.js` y `admin-usuarios.js` coinciden por SHA-256 con el build. En la base (solo lectura): 4 roles, `admin` con rol Administración, 133 fichas y 184 cuentas de Voceros, 74 medios y 6 creadoras intactos. No se modificaron datos existentes, DNS, Google Sheets ni otros proyectos.

### 2026-09-26 — Cambio de contraseña de la cuenta admin

- Alex pidió en el chat fijar la contraseña de la cuenta `admin` y entregó el valor. Se actualizó solo el hash de `admin` en producción mediante SSH, con `Auth::hashPassword` de la versión activa; la contraseña no se escribió en archivos, Git ni en esta bitácora.
