...
- Publicación externa: el commit técnico `783587f` (`Simplificar contador de venta de stands`) fue enviado a `origin/main`. El prevuelo de producción se detuvo antes de transferir archivos con `La validación SSH falló`; `ssh-add -l` devolvió `communication with agent failed`, por lo que la llave cifrada no estuvo disponible para autenticar. No hubo cambios remotos en cPanel.
