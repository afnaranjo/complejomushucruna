...
- Incidencia posterior: un navegador conservó la URL sin versión de `finados.js` y mostró el HTML nuevo con los valores iniciales `--`. La fuente y el servidor contenían el JavaScript correcto, pero faltaba invalidar la caché del recurso. Se versionaron `finados.css` y `finados.js` como `20260903-2` en `/finados/` y `/acceso-compra-stands/`; QA local: 25/25 pruebas, 71 archivos, 16 HTML y 407 referencias válidas.
