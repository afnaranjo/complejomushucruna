...
- Publicación externa: solo GitHub. **No está desplegado ni importado en producción**; requiere autorización de Alex para backend → frontend con la migración 018 y, después, ejecutar el importador en el servidor (simulación y luego `--apply`).
- Riesgos y pendientes: confirmar Radio Brisa / Brisa Stereo y Radio Estéreo San Miguel; las fichas de coordinación no tienen consentimientos ni foto hasta que el medio vincule su cuenta; «seguidores validados» se cargan desde la hoja como dato de coordinación.
