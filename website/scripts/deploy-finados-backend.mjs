...
export function backendReleaseFiles() {
  ...
  const files = tracked.stdout.split('\0').filter(Boolean).filter(file => /^(src\/[^/]+\.php|bin\/[^/]+\.php|migrations\/\d+_[a-z_]+\.sql|public\/(index\.php|\.htaccess)|resources\/(?:vocero|media|emprendedor)-consents\.json)$/.test(file)).sort();
