...
const videoSlotControls = () => VIDEO_SLOTS.map(slot => `<label class="admin-video-control"><span class="admin-video-check"><input type="checkbox" name="video_${slot}_enabled" value="1"><strong>Video ${slot}</strong></span><span class="admin-video-date"><span>Habilitado desde</span><input type="date" name="video_${slot}_enabled_at" aria-label="Fecha de habilitación del video ${slot}"></span></label>`).join('');
...
<script type="module" src="/assets/admin/admin.js?v=20260918-navigation-progress-1"></script>
...
<section class="pending-accounts" aria-labelledby="pending-accounts-title"><div class="records-heading"><div><h2 id="pending-accounts-title">Cuentas pendientes de ficha</h2><p>Las cuentas aparecen aquí apenas se crean. Cuando completen el formulario pasarán a Registros.</p></div><p data-pending-count>—</p></div><p class="feedback" data-pending-message role="status" aria-live="polite">Cargando cuentas…</p><div class="pending-accounts-table"><table><caption class="sr-only">Cuentas creadas que todavía no completan su ficha</caption><thead><tr><th scope="col">Correo</th><th scope="col">Creada</th><th scope="col">Estado</th><th scope="col"><span class="sr-only">Acciones</span></th></tr></thead><tbody data-pending-accounts></tbody></table></div></section>
...
<section class="admin-progress" aria-labelledby="admin-progress-title"><div class="admin-progress-heading"><div><h3 id="admin-progress-title">Progreso del vocero</h3><p>Registra la métrica validada y habilita los videos de forma gradual.</p></div><span data-admin-progress-summary>Sin actualizar</span></div><form data-progress-form><fieldset disabled><div class="admin-progress-grid"><label>Seguidores validados<input type="number" name="followers_count" min="0" max="1000000000" step="1" required></label><label>Nivel<select name="level" required>${PROGRESS_LEVELS.map((label, index) => `<option value="${index}">${index} · ${esc(label)}</option>`).join('')}</select></label><label>Semáforo<select name="traffic_light" required><option value="red">Rojo · En preparación</option><option value="yellow">Amarillo · En avance</option><option value="green">Verde · Listo</option></select></label><label>Kit<select name="kit_status" required><option value="pendiente">Pendiente de retiro</option><option value="retirado">Retirado</option></select></label></div><fieldset class="admin-video-controls" data-admin-video-controls><legend>Videos habilitados</legend><p class="admin-video-help">Marca cada video y registra la fecha desde la que puede cargarlo.</p><div class="admin-video-list">${videoSlotControls()}</div></fieldset><button class="button-primary" type="submit">Guardar progreso</button></fieldset></form><p class="feedback" data-progress-feedback role="status" aria-live="polite"></p><div class="admin-videos" data-admin-videos></div></section>
