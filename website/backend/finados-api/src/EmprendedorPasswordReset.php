...
require_once __DIR__ . '/EmprendedorAuth.php';
...
final class EmprendedorPasswordReset
{
    ...
    public function create(string $profilePublicId, int $adminId, string $ip): string
    {
        ...
        return $this->transaction(function () use ($profilePublicId, $adminId, $ip): string {
            ...
            $query = $this->pdo->prepare('SELECT a.id FROM emprendedor_accounts a JOIN emprendedor_profiles m ON m.account_id = a.id WHERE m.public_id = ? AND a.active = 1' . $this->rowLock());
            ...
            $count = $this->pdo->prepare('SELECT COUNT(*) FROM emprendedor_password_resets WHERE account_id = ? AND created_at > ?');
            $count->execute([$accountId, gmdate('Y-m-d H:i:s', $now - 900)]);
            if ((int) $count->fetchColumn() >= 5) throw new PasswordResetRateLimit();
            $raw = bin2hex(random_bytes(32));
            $this->pdo->prepare('UPDATE emprendedor_password_resets SET consumed_at = ? WHERE account_id = ? AND consumed_at IS NULL')->execute([$at, $accountId]);
            $this->pdo->prepare('INSERT INTO emprendedor_password_resets (account_id, token_hash, expires_at, created_by_admin_id, created_at) VALUES (?, ?, ?, ?, ?)')->execute([
                $accountId, hash_hmac('sha256', $raw, $this->config->hmacKey()), gmdate('Y-m-d H:i:s', $now + 1800), $adminId, $at,
            ]);
            $this->audit->log('emprendedor.password_reset_created', $adminId, 'emprendedor_profile', $profilePublicId, [], $ip);
            return $raw;
        });
    }
    ...
    {
        $ip = $this->ip($ip);
        EmprendedorAuth::password($password);
        ...
        $accepted = $this->transaction(function () use ($token, $password, $ip): bool {
            $now = ($this->clock)(); $at = gmdate('Y-m-d H:i:s', $now);
            $count = $this->pdo->prepare("SELECT COUNT(*) FROM audit_log WHERE event_type = 'emprendedor.password_reset_attempted' AND ip_hash = ? AND created_at > ?");
            ...
            $this->audit->log('emprendedor.password_reset_attempted', null, 'emprendedor_account', null, [], $ip);
            $query = $this->pdo->prepare('SELECT r.id, r.account_id, r.expires_at, r.consumed_at, a.public_id, a.active FROM emprendedor_password_resets r JOIN emprendedor_accounts a ON a.id = r.account_id WHERE r.token_hash = ?' . $this->rowLock());
            ...
            $this->pdo->prepare('UPDATE emprendedor_accounts SET password_hash = ?, updated_at = ? WHERE id = ?')->execute([$hash, $at, $row['account_id']]);
            $this->pdo->prepare('UPDATE emprendedor_password_resets SET consumed_at = ? WHERE account_id = ? AND consumed_at IS NULL')->execute([$at, $row['account_id']]);
            $this->audit->log('emprendedor.password_reset_consumed', null, 'emprendedor_account', $row['public_id'], [], $ip);
            return true;
        });
    ...
    private function transaction(callable $operation): mixed
    {
        ...
        try {
            if ($mysql) {
                ...
                $query->execute(['finados.emprendedor.password-reset']);
    ...
    private function logCleanupFailure(string $operation): void
    {
        ...
        try { error_log('Finados emprendedor password reset ' . $operation . ' cleanup failed.'); }
