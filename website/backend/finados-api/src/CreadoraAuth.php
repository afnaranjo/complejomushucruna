...
final class CreadoraAuth
{
    ...
    public function register(string $email, #[\SensitiveParameter] string $password, bool $privacyAcknowledged, string $ip): array
    {
        ...
        try {
            ...
            $insert = $this->pdo->prepare('INSERT INTO creadora_accounts (public_id, email_enc, email_idx, password_hash, privacy_version, privacy_hash, privacy_acknowledged_at, active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?)');
            $insert->execute([
            ...
            $this->audit->log('creadora_account.registered', null, 'creadora_account', $account['public_id'], [], $ip);
            $this->commit();
            $transactionStarted = false;
            ...
            $existing = $this->pdo->prepare('SELECT id, public_id, active FROM creadora_accounts WHERE email_idx = ?');
            $existing->execute([$emailIndex]);
            $archived = $existing->fetch();
            if ($archived !== false && (int) $archived['active'] === 0) {
                $linked = $this->pdo->prepare('SELECT 1 FROM creadoras WHERE account_id = ? LIMIT 1');
                $linked->execute([(int) $archived['id']]);
                if ($linked->fetchColumn() === false) {
                    $this->pdo->beginTransaction();
                    try {
                        $this->pdo->prepare('UPDATE creadora_accounts SET password_hash = ?, privacy_version = ?, privacy_hash = ?, privacy_acknowledged_at = ?, active = 1, updated_at = ? WHERE id = ? AND active = 0')
                        ...
                        $this->audit->log('creadora_account.reactivated', null, 'creadora_account', $archived['public_id'], [], $ip);
                        $this->pdo->commit();
    ...
    public function login(string $email, #[\SensitiveParameter] string $password, string $ip): array
    {
        ...
        Http::startSession($this->config, 'creadora');
        $ipHash = $this->ipHash($ip);
        $emailIndex = $this->crypto->lookup($email);
        $now = ($this->clock)();
        $mysql = $this->pdo->getAttribute(PDO::ATTR_DRIVER_NAME) === 'mysql';
        $ipLockName = $mysql ? 'finados.creadora.ip.' . substr($ipHash, 0, 46) : null;
        ...
        try {
            ...
            $transactionStarted = true;
            $sqlite = !$mysql;
            $statement = $this->pdo->prepare('SELECT * FROM creadora_accounts WHERE email_idx = ?' . ($sqlite ? '' : ' FOR UPDATE'));
            $statement->execute([$emailIndex]);
            $account = $statement->fetch();
            $emailAttempts = $this->pdo->prepare('SELECT succeeded, attempted_at FROM creadora_login_attempts WHERE email_idx = ? AND attempted_at >= ? ORDER BY attempted_at, id');
            $emailAttempts->execute([$emailIndex, gmdate('Y-m-d H:i:s', $now - 1800)]);
            $ipAttempts = $this->pdo->prepare('SELECT succeeded, attempted_at FROM creadora_login_attempts WHERE ip_hash = ? AND attempted_at >= ? ORDER BY attempted_at, id');
            ...
            if (!$blocked) {
                ...
                $record = $this->pdo->prepare('INSERT INTO creadora_login_attempts (email_idx, ip_hash, succeeded, attempted_at) VALUES (?, ?, ?, ?)');
                ...
                if ($valid) {
                    ...
                    $this->pdo->prepare('UPDATE creadora_accounts SET password_hash = ?, last_login_at = ?, updated_at = ? WHERE id = ?')
    ...
    public function requireUser(): array
    {
        Http::startSession($this->config, 'creadora');
        $now = ($this->clock)();
        if (!isset($_SESSION['creadora_account_id'], $_SESSION['authenticated_at'], $_SESSION['last_activity'], $_SESSION['credential_version'])
        ...
        $statement = $this->pdo->prepare('SELECT id, public_id, email_enc, active, password_hash FROM creadora_accounts WHERE id = ?');
        $statement->execute([$_SESSION['creadora_account_id']]);
        $account = $statement->fetch();
    ...
    private function start(array $account, string $email, int $now): array
    {
        ...
        $_SESSION = [
            'creadora_account_id' => (int) $account['id'], 'authenticated_at' => $now, 'last_activity' => $now,
            'credential_version' => $this->credentialVersion($account['password_hash']), 'csrf' => bin2hex(random_bytes(32)),
        ];
        return ['user' => ['id' => (int) $account['id'], 'public_id' => $account['public_id'], 'email' => $email, 'role' => 'creadora'], 'csrf' => $_SESSION['csrf']];
    }
    ...
    {
        return ['id' => (int) $account['id'], 'public_id' => $account['public_id'], 'email' => $this->crypto->decrypt($account['email_enc']), 'role' => 'creadora'];
    }
    ...
    private function isEmailDuplicateViolation(\PDOException $error): bool
    {
        ...
        if ($driver === 'sqlite') {
            return in_array((string) $error->getCode(), ['19', '23000'], true)
                && preg_match('/UNIQUE constraint failed: creadora_accounts\\.email_idx$/D', $error->getMessage()) === 1;
        }
    ...
    {
        return preg_match("/for key ['`](?:email_idx|creadora_accounts\\.email_idx)['`]$/D", $message) === 1;
    }

    private function logLoginCleanupFailure(string $operation): void
    {
        try { error_log('Finados creadora login ' . $operation . ' cleanup failed.'); }
