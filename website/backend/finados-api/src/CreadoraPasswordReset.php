...
require_once __DIR__ . '/CreadoraAuth.php';
...
final class CreadoraPasswordReset
{
    ...
    public function create(string $profilePublicId, int $adminId, string $ip): string
    {
        ...
        return $this->transaction(function () use ($profilePublicId, $adminId, $ip): string {
            ...
            $query = $this->pdo->prepare('SELECT a.id FROM creadora_accounts a JOIN creadoras m ON m.account_id = a.id WHERE m.public_id = ? AND a.active = 1' . $this->rowLock());
            ...
            $count = $this->pdo->prepare('SELECT COUNT(*) FROM creadora_password_resets WHERE account_id = ? AND created_at > ?');
            $count->execute([$accountId, gmdate('Y-m-d H:i:s', $now - 900)]);
            if ((int) $count->fetchColumn() >= 5) throw new PasswordResetRateLimit();
            $raw = bin2hex(random_bytes(32));
            $this->pdo->prepare('UPDATE creadora_password_resets SET consumed_at = ? WHERE account_id = ? AND consumed_at IS NULL')->execute([$at, $accountId]);
            $this->pdo->prepare('INSERT INTO creadora_password_resets (account_id, token_hash, expires_at, created_by_admin_id, created_at) VALUES (?, ?, ?, ?, ?)')->execute([
                $accountId, hash_hmac('sha256', $raw, $this->config->hmacKey()), gmdate('Y-m-d H:i:s', $now + 1800), $adminId, $at,
            ]);
            $this->audit->log('creadora.password_reset_created', $adminId, 'creadora', $profilePublicId, [], $ip);
            return $raw;
        });
    }
    ...
    {
        $ip = $this->ip($ip);
        CreadoraAuth::password($password);
        ...
        $accepted = $this->transaction(function () use ($token, $password, $ip): bool {
            $now = ($this->clock)(); $at = gmdate('Y-m-d H:i:s', $now);
            $count = $this->pdo->prepare("SELECT COUNT(*) FROM audit_log WHERE event_type = 'creadora.password_reset_attempted' AND ip_hash = ? AND created_at > ?");
            ...
            $this->audit->log('creadora.password_reset_attempted', null, 'creadora_account', null, [], $ip);
            $query = $this->pdo->prepare('SELECT r.id, r.account_id, r.expires_at, r.consumed_at, a.public_id, a.active FROM creadora_password_resets r JOIN creadora_accounts a ON a.id = r.account_id WHERE r.token_hash = ?' . $this->rowLock());
            ...
            $this->pdo->prepare('UPDATE creadora_accounts SET password_hash = ?, updated_at = ? WHERE id = ?')->execute([$hash, $at, $row['account_id']]);
            $this->pdo->prepare('UPDATE creadora_password_resets SET consumed_at = ? WHERE account_id = ? AND consumed_at IS NULL')->execute([$at, $row['account_id']]);
            $this->audit->log('creadora.password_reset_consumed', null, 'creadora_account', $row['public_id'], [], $ip);
            return true;
        });
    ...
    private function transaction(callable $operation): mixed
    {
        ...
        try {
            if ($mysql) {
                ...
                $query->execute(['finados.creadora.password-reset']);
    ...
    private function logCleanupFailure(string $operation): void
    {
        ...
        try { error_log('Finados creadora password reset ' . $operation . ' cleanup failed.'); }
