...
test('build multi-origen publica todas las dependencias ESM y renueva los clientes', async t => {
  ...
  for (const [route, script] of [
    ...
    const version = script === 'admin/admin.js' ? '20260923-admin-sidebar-1' : script === 'finados/vocero-verification.js' ? '20260918-multi-origin-1' : '20260918-navigation-progress-1';
  ...
  for (const file of files) {
    ...
    for (const [, , query] of [...html.matchAll(/(?:src|href)="(\/assets\/[^"?]+)(?:\?([^" ]+))?"/g)]
      ...
      assert.equal(new URLSearchParams(query).get('v'), '20260923-admin-sidebar-1', `${file}: admin/admin.js`);
