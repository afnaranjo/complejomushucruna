...
test('development build uses its configured loopback API; production refuses overrides', async () => {
  ...
  try {
    ...
    for (const file of files.filter((file) => !file.startsWith('admin/') && !file.startsWith('assets/admin/') && !/^finados\/(?:voceros|medios|emprendedores)\/(acceso|mi-registro|restablecer)\//.test(file) && !['assets/finados/vocero-portal.js', 'assets/finados/media-portal.js', 'assets/finados/emprendedor-portal.js'].includes(file))) {
