...
test('agrega un aviso de cookies compacto en páginas públicas y excluye admin', async () => {
  ...
  for (const htmlFile of htmlFiles) {
    ...
    if (htmlFile.startsWith('admin/') || /^finados\/(?:voceros|medios|emprendedores|creadoras|mfs)\/(acceso|mi-registro|restablecer|acreditacion)\//.test(htmlFile)) {
...
test('agrega Meta Pixel en páginas públicas y excluye áreas privadas', async () => {
  ...
  for (const htmlFile of htmlFiles) {
    ...
    const privateArea = htmlFile.startsWith('admin/')
      || /^finados\/(?:voceros|emprendedores|creadoras|mfs)\/(acceso|mi-registro|restablecer|verificar)\//.test(htmlFile)
...
test('genera las rutas institucionales y el archivo histórico', async () => {
  ...
  for (const expected of [
    ...
    'finados/emprendedores/index.html',
    'finados/emprendedores/verificar/index.html',
    'admin/emprendedores/index.html',
...
test('la portada adopta la cabecera de venta de Finados y simplifica la navegación', async () => {
  ...
  assert.equal((mainNavigation.match(/main-nav__action--stands/g) || []).length, 1);
  assert.ok(
    mainNavigation.indexOf('>FINADOS 2026</a>') < mainNavigation.indexOf('>VENTA DE STANDS</a>'),
    'Venta de stands debe quedar dentro del submenú de Finados 2026',
  );
  assert.match(
    mainNavigation,
  ...
  const expectedOrder = ['INICIO', 'FINADOS 2026', 'ACREDITACIÓN DE MEDIOS', 'TOUR VIRTUAL', 'GRANJA', 'NOSOTROS'];
...
test('publica una acreditación de medios completa, limitada por fecha y respaldada en servidor', async () => {
  ...
  assert.match(page, /\/assets\/media-accreditation\.css\?v=20260916-4/);
...
test('genera las páginas de Finados y ordena sus subpáginas en un menú desplegable', async () => {
  ...
  assert.match(stands, /\/assets\/finados\/finados\.js\?v=20260918-navigation-progress-1/);
