---
titulo: "Plan operativo digital Feria de Finados 2026"
responsable: "coordinación de marketing y comunicación"
estado: en-revision
ultima_actualizacion: 2026-08-24
fuente: "decisiones de Alex Naranjo, auditoría histórica Meta 2021-2026 y expediente integral de la feria"
confidencialidad: restringido
tags: [feria-finados-2026, estrategia-digital, plan-operativo]
---

# Plan operativo digital Feria de Finados 2026

## Decisión ejecutiva

La estrategia comienza a ejecutarse internamente el **martes 25 de agosto de 2026** y trabaja con una ventana operativa asumida de feria del **jueves 29 de octubre al lunes 2 de noviembre de 2026**.

Esta ventana está aprobada como supuesto de planificación. No queda autorizada para publicación hasta que Dirección confirme que es la fecha pública oficial.

El presupuesto de **USD 4.000** se destina exclusivamente a pauta en redes sociales. Influencers, radio, televisión y prensa tienen presupuestos propios administrados por sus responsables; marketing digital coordina narrativa, calendario, enlaces y evidencia, pero no administra ni descuenta esos valores de los USD 4.000.

Este documento operacionaliza la [[2026-08-24_estrategia-digital-finados-2026_v01|estrategia digital provisional]] y se apoya en la [[../../../../13_datos-medicion/2026-08-24_auditoria-meta-2021-2026/README|auditoría histórica de Meta]]. No autoriza publicar, activar pauta ni cambiar cuentas externas.

## Tesis y prioridad

> **La música atrae. La experiencia total diferencia. La confianza convierte. La operación cumple.**

Orden de prioridades:

1. Convertir el interés en visitas o compras verificables.
2. Explicar por qué vale la pena elegir Mushuc Runa frente a otras opciones del feriado.
3. Reducir incertidumbre sobre precios, agenda, movilidad, servicios y seguridad.
4. Usar la cobertura en vivo para orientar y demostrar la experiencia, no para saturar.
5. Conservar datos y aprendizajes para la siguiente edición.

## Evidencia que sostiene el plan

### Verificado

- La auditoría local revisó 1.414 publicaciones de `Finados Mushuc Runa` y `Carnavales Mushuc Runa`.
- El video superó de forma consistente la mediana de fotografía.
- Música y artistas fueron el gancho más fuerte; la información útil también produjo publicaciones destacadas.
- Finados 2025 concentró 179 publicaciones en cinco días, 35,8 por día, con baja mediana de interacción; más volumen no significó mejor desempeño.
- La pauta histórica disponible es parcial y las compras atribuidas por Meta no están conciliadas con boletería.
- La pregunta de Carnaval 2022 sobre qué artista debería presentarse fue la publicación número 1 de consideración de esa edición: 847 interacciones y 535 comentarios.
- Las candidaturas y finalistas de `Rey Pan` y `Srta. Colada Morada` sumaron 6.876 interacciones en doce publicaciones de la página principal durante Finados 2025; su mediana de 358 superó ampliamente la mediana de 40 de la fase de conversión, pero no prueba ventas.
- En 2025 existieron ferias y programación artística gratuita del Municipio y del Gobierno Provincial durante el mismo feriado. Para 2026 es una amenaza probable todavía sin anuncio oficial encontrado.

### Confirmado por Alex para este plan

- Inicio del trabajo digital: 2026-08-25.
- Ventana operativa asumida: 2026-10-29 a 2026-11-02.
- Pauta social: USD 4.000.
- Presupuestos de influencers y medios tradicionales: separados y fuera del control de pauta digital.
- Entrada general de adulto: USD 3.
- Acceso al megaescenario: pago adicional con precio por confirmar.

### Por confirmar antes de publicar o vender

- Ratificación pública de las fechas.
- Nombre oficial de la edición, precios, inclusiones, aforo y condiciones.
- Precio, aforo y condiciones del megaescenario; niños, reingreso, parqueadero y cargos adicionales.
- Artistas, experiencias y horarios aprobados para comunicación.
- Dominio o destino seguro y canal de compra.
- Responsable final de aprobación y turnos reales del equipo.

## Objetivo de negocio y embudo

El resultado superior es **asistencia rentable y satisfecha**. La pauta se evalúa contra la fuente más cercana al negocio que sea técnicamente confiable.

| Etapa | Progreso del público | CTA principal | KPI operativo | Fuente de verdad |
|---|---|---|---|---|
| Descubrimiento | Reconoce una propuesta relevante | Ver/guardar | alcance útil, reproducción, guardados/compartidos | Meta y analítica de contenido |
| Consideración | Encuentra un motivo concreto para asistir | Ver agenda/planificar | visita útil, consumo de agenda, conversación con intención | analítica o mensajería |
| Conversión | Elige fecha/producto y completa la acción | Comprar/reservar | compra conciliada o conversación calificada | boletería/caja o registro de atención |
| Servicio | Sabe cómo llegar y disfrutar | Consultar guía | uso de información, dudas resueltas, incidentes evitados | sitio, atención y operación |
| Memoria | Recomienda y deja evidencia | Evaluar/compartir | satisfacción, contenido autorizado y repetición | encuesta y cierre |

Si el evento `purchase` no pasa una compra de prueba completa, la campaña no se optimiza ni se reporta como ventas. Se usa temporalmente una conversación calificada o una visita útil, con su limitación declarada.

## Calendario maestro

| Fase | Fechas | Objetivo | Salida obligatoria | Pauta |
|---|---|---|---|---:|
| 0. Fundamentos | 25 ago–2 sep | Dejar listo gobierno, activos, destino, tracking y respuesta | fuente única, responsables, QA, tres conceptos piloto | USD 0 |
| 1. Expectativa | 3–16 sep | Construir públicos y participación sin depender solo del cartel | convocatoria/voto, registro prioritario y memoria | USD 400 |
| 2. Revelaciones | 17 sep–1 oct | Transformar atención en preventa | ganador, confirmaciones, itinerarios y primeros lotes | USD 800 |
| 3. Conversión | 2–21 oct | Asegurar compradores antes de la presión competitiva tardía | producto por noche, precio total, prueba y CTA | USD 1.600 |
| 4. Urgencia útil | 22–28 oct | Cerrar decisión y reducir fricción | agenda final, disponibilidad real, movilidad y FAQ | USD 800 |
| 5. Feria en vivo | 29 oct–2 nov | Orientar, proteger reputación y capturar prueba social | información fijada, alertas, recap diario | USD 200 |
| Reserva controlada | por liberar | Reforzar una señal demostrada o cubrir contingencia | decisión registrada con motivo y KPI | USD 200 |
| **Total** | 3 sep–2 nov |  |  | **USD 4.000** |

La reserva no es dinero libre. Solo se libera por resultado verificable, necesidad operativa o cambio aprobado; si no existe una señal confiable, no se gasta por cumplir el presupuesto.

## Sprint de arranque · 25 de agosto a 2 de septiembre

### 25 de agosto

- Reunión de 45 minutos con Dirección, marketing, comunicación, producción, ventas y datos.
- Ratificar la fecha pública, oferta, canal de conversión y aprobador final.
- Confirmar qué página es centro oficial y quién responde mensajes.
- Aprobar la arquitectura comercial: general adultos USD 3, megaescenario adicional, combo y posibles pases.
- Aprobar o descartar la convocatoria `Camino al Megaescenario` y reservar un slot real antes de prometerlo.
- Crear inventario de videos, fotografías, testimonios y permisos de uso; no mover activos aprobados.

### 26 de agosto

- Probar destino, formulario/mensajería y compra de punta a punta sin activar anuncios.
- Definir UTMs, registro de consultas y exportación diaria de ventas.
- Acordar el SLA interno de respuesta según turnos reales.

### 27 y 28 de agosto

- Producir tres pilotos verticales, cada uno con un concepto distinto: música, experiencia completa y confianza/servicio.
- Preparar adaptación 4:5 y una pieza consultable por piloto.
- Revisar derechos, nombres, datos, subtítulos, zonas seguras y CTA.

### 29 de agosto a 2 de septiembre

- Publicar orgánicamente solo las piezas que superen aprobación.
- Medir retención, compartidos, guardados, preguntas e intención; no declarar ganador por likes.
- Elegir los conceptos que entran a pauta el 3 de septiembre.

## Sistema creativo

| Concepto | Tensión que resuelve | Prueba necesaria | Formato principal | CTA por etapa |
|---|---|---|---|---|
| Música que convoca | “Quiero saber si el evento vale el viaje” | artista/horario confirmado o archivo real autorizado | reel 9:16 | ver agenda/comprar |
| Cinco días, varios planes | “No sé qué día o plan elegir” | itinerario real por público y día | video + carrusel | armar mi plan |
| Territorio y comunidad | “Busco algo distinto, no otra feria genérica” | voces, oficios, gastronomía y contexto con autorización | microdocumental | conocer/guardar |
| Confianza práctica | “Me preocupa llegar, pagar o encontrar servicios” | mapa, precio, parqueo, baños, accesibilidad y atención verificados | carrusel/estático | consultar guía |
| Comercio vivo | “Quiero descubrir o vender productos” | expositor confirmado, producto y condiciones | reel/UGC autorizado | visitar/aplicar |
| La gente elige | “Quiero ser parte de la programación” | artistas elegibles, voto auditado y resultado presencial | reel + landing | postular/votar/comprar |
| Preventa transparente | “Quiero asegurar mi noche sin cobros sorpresa” | producto, total, cupo y condiciones | video + carrusel | comprar combo |

Reglas:

- Diseñar primero en 9:16 y recomponer a 4:5; no limitarse a recortar.
- Comunicar valor aun sin sonido, con subtítulos y texto breve.
- Una pieza, una promesa, una prueba y un CTA.
- No usar texto generado por IA como arte final.
- No publicar cifras, artistas, servicios o urgencia sin evidencia vigente.
- No convertir símbolos culturales en decoración; la aprobación comunitaria es un gate.

El detalle de producción está en [[../02_contenido/2026-08-24_sistema-contenidos-finados-2026_v01|sistema de contenidos]].

La respuesta a eventos gratuitos, la recuperación de `Rey Pan`/`Srta. Colada Morada`, la convocatoria artística y la preventa se desarrollan en [[2026-08-24_estrategia-competitiva-preventa-participacion-finados-2026_v01|la estrategia competitiva y de participación]].

## Arquitectura de pauta

Meta es el canal pagado principal porque allí existe evidencia histórica, audiencias y capacidad creativa. Con USD 4.000 no se divide la inversión entre plataformas sin una hipótesis y medición comparables.

- **Campaña A · Participación:** máximo USD 200 para convocatoria/voto; el alcance principal debe venir de los artistas y sus comunidades.
- **Campaña B · Preventa/resultado:** USD 3.400 para registro prioritario, combos, pases y venta por noche. Optimiza a compra únicamente si pasa el gate técnico; de lo contrario, a conversación calificada o visita útil.
- **Campaña C · Servicio en vivo:** USD 200, geografía cercana y datos operativos confirmados.
- **Reserva:** USD 200, no asignada hasta decisión documentada.

El retargeting vive dentro de la campaña de resultado o recibe un conjunto separado únicamente si tiene volumen suficiente. No se crean microcampañas por interés, formato, artista o cada día de feria.

Detalle: [[../03_pauta/2026-08-24_plan-pauta-social-usd4000-finados-2026_v01|plan de pauta social]].

## Integración con medios e influencers

Digital no administra sus presupuestos. Sí entrega y controla:

- narrativa y hechos aprobados;
- calendario de revelaciones y embargos;
- kit de piezas, B-roll, fotos, logos y preguntas frecuentes;
- enlace, QR, palabra clave o código por aliado;
- registro de salida, evidencia y respuesta generada;
- protocolo de corrección cuando cambie un dato.

Detalle: [[../04_pr-medios/2026-08-24_protocolo-integracion-medios-finados-2026_v01|protocolo de integración de medios e influencers]].

## Equipo y gobierno

La estructura visual entregada por Alex corresponde a “Estructura finados 2025” y se usa como referencia de equipo. El mapeo 2026 y las responsabilidades digitales deben ratificarse antes del 26 de agosto.

Cadena observada: Luis Lara → Andrés Naranjo → Edwin Masabanda, con Alex Naranjo en asesoría a Edwin. Desde Edwin se coordinan diseño/audiovisual, comunicación y producción.

El organigrama, la propuesta RACI y los vacíos están documentados en [[../2026-08-24_equipo-operativo-marketing-finados-2026_v01|equipo operativo de marketing]].

## Interconexión con todos los frentes

| Frente dueño | Entrega a marketing digital | Marketing devuelve | Gate |
|---|---|---|---|
| [[../../00_direccion-control/acta-constitucion|Dirección]] | fecha pública, objetivos, aprobador y decisiones | tablero, escenarios y alertas | ninguna campaña sin A final |
| [[../../01_concepto-experiencia/README|Concepto/experiencia]] | promesa, públicos y reglas culturales | narrativa y prueba de comprensión | concepto validado |
| [[../../02_programacion-atracciones/README|Programación]] | ficha confirmada por artista/actividad | anuncio, agenda y control de cambios | `aprobado_para_publicar` |
| [[../../03_expositores/README|Expositores]] | oferta, cupo, casos y confirmaciones | captación y visibilidad | condiciones aprobadas |
| [[../../04_patrocinios-alianzas/README|Patrocinios]] | obligaciones, categorías y logos | plan de exposición y evidencia | contrato/uso aprobado |
| [[../README|Marketing/comunicación]] | estrategia, producción, comunidad y pauta | demanda y aprendizaje | QA de campaña |
| [[../../06_ventas-entradas/README|Ventas/entradas]] | productos, precio, inventario y ventas reales | tráfico medido y conciliación | compra de prueba |
| [[../../07_produccion-logistica/README|Producción/logística]] | mapa, rutas, parqueo, señalética y cambios | guías y alertas públicas | información operativa validada |
| [[../../08_permisos-seguridad/README|Permisos/seguridad]] | aforo, restricciones y protocolos | comunicación preventiva/crisis | aprobación de seguridad |
| [[../../09_finanzas-compras/README|Finanzas]] | tope, margen y pagos aprobados | gasto diario y proyección | presupuesto disponible |
| [[../../10_tecnologia-datos/README|Tecnología/datos]] | destino seguro, integraciones y calidad | UTMs, requisitos y anomalías | tracking operativo |
| [[../../11_personal-proveedores/README|Personal/proveedores]] | turnos, voceros y SLA reales | demanda prevista y necesidades | capacidad de respuesta |
| [[../../12_servicio-experiencia/README|Servicio]] | FAQ y resolución de problemas | temas, volumen y percepción | respuestas verificadas |
| [[../../13_riesgos-contingencias/README|Riesgos]] | escenarios y autoridad de escalamiento | señales tempranas y mensajes | protocolo activo |
| [[../../14_operacion-en-vivo/README|Operación en vivo]] | centro de control y cambios por hora | publicación priorizada y escucha | fuente única en vivo |
| [[../../15_cierre-aprendizajes/README|Cierre]] | ventas, asistencia, costos y satisfacción | informe de marketing y biblioteca | conciliación final |

## Gates de activación

1. **G-D1 · Gobierno:** fechas públicas, oferta, precio, inventario, aprobador y responsables.
2. **G-D2 · Seguridad digital:** destino seguro, propiedad, SSL y plan alterno.
3. **G-D3 · Medición:** UTMs, eventos, deduplicación y compra o conversación de prueba.
4. **G-D4 · Operación:** atención, FAQ, movilidad, servicios y escalamiento.
5. **G-D5 · Creatividad:** derechos, datos, formatos, marca, cultura y política.

Si falla G-D2 o G-D3, no se activa pauta de conversión. Si falla un dato de G-D1 o G-D4, se retira esa afirmación de la pieza; no se improvisa.

## Ritmo de gestión

- Hasta el 1 de octubre: comité digital semanal de 30 minutos.
- Del 2 al 21 de octubre: lunes y jueves, 30 minutos.
- Del 22 al 28 de octubre: corte diario de 15 minutos.
- Del 29 de octubre al 2 de noviembre: enlace permanente con el centro de control y dos cortes formales al día.
- 3 de noviembre: corte preliminar.
- 2 de diciembre: cierre conciliado y aprendizajes.

## Criterio de éxito

El plan funciona si convierte demanda en resultado verificable sin prometer más de lo que la operación puede entregar. Alcance, reproducciones y reacciones son diagnósticos; no sustituyen ventas, asistencia, satisfacción ni margen.

## Navegación

- [[README|Índice de estrategia]]
- [[../README|Marketing y comunicación]]
- [[../../00_direccion-control/registro-decisiones|Decisiones de la feria]]
- [[../../../../_pendientes|Pendientes ejecutivos]]
