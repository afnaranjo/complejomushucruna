---
titulo: "Acta de constitución Feria de Finados 2026"
responsable: "dirección integral de la feria"
estado: borrador
ultima_actualizacion: 2026-08-28
fuente: interna
confidencialidad: restringido
tags:
  - direccion
  - feria-finados-2026
---

# Acta de constitución

## Propósito

Diseñar, comercializar, promocionar, producir y operar integralmente la Feria de Finados 2026 en el Complejo Mushuc Runa.

## Supuestos aprobados para planificación

- Inicio del trabajo digital: 2026-08-25.
- Programación de feria: confirmada en una fuente de acceso controlado. Este repositorio público usa Día 1–Día 5; las fechas exactas requieren autorización antes de incorporarse a una versión pública.
- Presupuesto: USD 4.000 exclusivamente para pauta en redes sociales.
- Influencers, radio, televisión y prensa administran presupuestos separados; el frente digital coordina mensaje, calendario y medición.
- Entrada general para adultos: USD 3.
- El acceso al megaescenario se comercializa adicionalmente; precio, capacidad, noches e inclusiones siguen por confirmar.
- Objetivo de planificación: 200.000 asistentes; antes de publicarlo y usarlo como compromiso se debe definir si significa visitas acumuladas o personas únicas y validar la capacidad por día, hora y zona.
- Producto digital: guía móvil informativa con mapa y ubicaciones, sin backend, cuentas ni transacciones; implementación y costo aún pendientes.
- Dirección de posicionamiento: recuperar la identidad de feria comercial, familiar y cultural desde la tradición. `¡LEGADO QUE NOS UNE!` es el eslogan gráfico oficial y `ES TRADICIÓN` la plataforma comunicativa para videos, reels, anuncios y contenidos; cada ejecución mantiene sus gates culturales, de marca, operativos y editoriales.
- Cartel artístico: información confidencial; ningún nombre está autorizado para difusión hasta pasar contratos, programación, oferta, derechos y embargo.

La programación interna está confirmada; la versión publicable de fechas todavía requiere autorización de Dirección.

## Por confirmar con dirección

- Objetivo principal y objetivos secundarios.
- Fechas de montaje y desmontaje, y ratificación pública de apertura/cierre.
- Público objetivo, aforo y definición medible de la meta de 200.000 asistentes.
- Metas de ingresos, utilidad, expositores, patrocinadores y reputación.
- Precio y capacidad del megaescenario; precio infantil; reingreso, estacionamiento, cargos, descuentos y política de cambios o devoluciones.
- Presupuesto integral y reservas de contingencia fuera de los USD 4.000 de pauta social.
- Alcance incluido y excluido.
- Director general, sponsor y responsables de aprobación.
- Entidades organizadoras y naturaleza de sus relaciones.

## Criterios de éxito

| Dimensión | Indicador | Línea base | Meta | Fuente | Responsable |
|---|---|---:|---:|---|---|
| Asistencia | visitas validadas acumuladas; personas únicas como corte separado | por confirmar | 200.000 para planificación | sistema de acceso | por asignar |
| Comercial | ingresos totales | por confirmar | por confirmar | finanzas | por asignar |
| Expositores | ocupación y satisfacción | por confirmar | por confirmar | control de expositores | por asignar |
| Marketing | conversiones atribuibles | por confirmar | por confirmar | analítica/ventas | por asignar |
| Experiencia | satisfacción/NPS | por confirmar | por confirmar | encuesta | por asignar |
| Seguridad | incidentes críticos | por confirmar | 0 | centro de control | por asignar |
| Finanzas | desviación presupuestaria | por confirmar | por confirmar | presupuesto maestro | por asignar |

## Aprobación G0

- Estado: pendiente.
- Aprueba:
- Fecha:
- Evidencia:

[[../README|Volver al centro de la feria]]
